...
def batch_upload_handle(request):
    ...
    for i,image_name in enumerate(image_names):
        ...
        img_path = '%s/v0/segmentation/scaledown8_png/%s'%(settings.BASE_DIR,image_name_old)
        ...
        apply_color_filter_to_image(image_name_old, image_name, item_folder, filtered_dir, filted_mask_dir, hole_size=2000, object_size=3000)
        t.refresh(track_count)
        track_count += 1
        ...
        overlap = 400
        filter_rate = 0.10
        cutter = Cutter(slide_list, file_dir, save_patch_dir, sample_type=sample_type)
